...
function buildContainerArgs(
  ...
  // Claude CLI args — stream-json emits one typed JSON event per line
...
export async function runContainerAgent(
  ...
  return new Promise((resolve) => {
    ...
    // JSONL parsing: each stdout line is a typed claude CLI event
    let lineBuffer = '';
    ...
    container.stdout.on('data', (data) => {
      ...
      for (const line of lines) {
        ...
        if (transcriptLines.length < CONTAINER_MAX_OUTPUT_SIZE / 200) {
          ...
          if (event.type === 'system' && event.subtype === 'init') {
            ...
            if (onOutput) {
              const output: ContainerOutput = {
                status: (event.is_error as boolean) ? 'error' : 'success',
                result: resultText,
                newSessionId,
                totalCostUsd,
              };
    ...
    container.stderr.on('data', (data) => {
      ...
      // Timeout only resets on actual result events in stdout.
    ...
    container.on('close', (code) => {
      ...
      // Write transcript and session result to IPC dir for the receiving agent
      const ipcDir = resolveGroupIpcPath(group.folder);
      fs.mkdirSync(ipcDir, { recursive: true });
      const transcriptPath = path.join(ipcDir, 'transcript.jsonl');
      ...
      const logLines = [
        ...
        `Transcript Lines: ${transcriptLines.length}`,
        ``,
      ];
